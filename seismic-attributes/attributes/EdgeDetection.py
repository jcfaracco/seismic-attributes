    ...
    def gradient_structure_tensor(self, darray, kernel=(3, 3, 9),
        ...
        def operation(gi2, gj2, gk2, gigj, gigk, gjgk):
            if USE_CUPY and self._use_cuda:
                ...
                eigs = cp.sorgst(cp.linalg.eigvalsh(gst))
            else:
                np.seterr(all='ignore')
        ...
        if USE_CUPY and self._use_cuda:
            ...
            gj2 = (gj * gj).map_blocks(cundi.uniform_filter, size=kernel,
                                       dtype=darray.dtype)
            gk2 = (gk * gk).map_blocks(cundi.uniform_filter, size=kernel,
                                       dtype=darray.dtype)
            gigj = (gi * gj).map_blocks(cundi.uniform_filter, size=kernel,
                                        dtype=darray.dtype)
            gigk = (gi * gk).map_blocks(cundi.uniform_filter, size=kernel,
                                        dtype=darray.dtype)
            gjgk = (gj * gk).map_blocks(cundi.uniform_filter, size=kernel,
                                        dtype=darray.dtype)
        else:
            gi2 = (gi * gi).map_blocks(ndi.uniform_filter, size=kernel,
                                       dtype=darray.dtype)
            gj2 = (gj * gj).map_blocks(ndi.uniform_filter, size=kernel,
                                       dtype=darray.dtype)
            gk2 = (gk * gk).map_blocks(ndi.uniform_filter, size=kernel,
                                       dtype=darray.dtype)
            gigj = (gi * gj).map_blocks(ndi.uniform_filter, size=kernel,
                                        dtype=darray.dtype)
            gigk = (gi * gk).map_blocks(ndi.uniform_filter, size=kernel,
                                        dtype=darray.dtype)
            gjgk = (gj * gk).map_blocks(ndi.uniform_filter, size=kernel,
                                        dtype=darray.dtype)
    ...
    def volume_curvature(self, darray_il, darray_xl, dip_factor=10,
        ...
        if USE_CUPY and self._use_cuda:
            ...
            uy = uy.map_blocks(cundi.uniform_filter, size=kernel,
                               dtype=ux.dtype)
            uz = uz.map_blocks(cundi.uniform_filter, size=kernel,
                               dtype=ux.dtype)
            vx = vx.map_blocks(cundi.uniform_filter, size=kernel,
                               dtype=ux.dtype)
            vy = vy.map_blocks(cundi.uniform_filter, size=kernel,
                               dtype=ux.dtype)
            vz = vz.map_blocks(cundi.uniform_filter, size=kernel,
                               dtype=ux.dtype)
        else:
            ux = ux.map_blocks(ndi.uniform_filter, size=kernel,
                               dtype=ux.dtype)
            uy = uy.map_blocks(ndi.uniform_filter, size=kernel,
                               dtype=ux.dtype)
            uz = uz.map_blocks(ndi.uniform_filter, size=kernel,
                               dtype=ux.dtype)
            vx = vx.map_blocks(ndi.uniform_filter, size=kernel,
                               dtype=ux.dtype)
            vy = vy.map_blocks(ndi.uniform_filter, size=kernel,
                               dtype=ux.dtype)
            vz = vz.map_blocks(ndi.uniform_filter, size=kernel,
                               dtype=ux.dtype)
