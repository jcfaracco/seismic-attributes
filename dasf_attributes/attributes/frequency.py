...
class Frequency(BaseAttributes):
    ...
    def cwt_ormsby(self, darray, freqs, sample_rate=4, preview=None):
        ...
        def wavelet(freqs, sample_rate):
            ...
            term1 = ((((np.pi * f4) ** 2) / ((np.pi * f4) - (np.pi * f3))) *
                     np.sinc(np.pi * f4 * t) ** 2)
            term2 = ((((np.pi * f3) ** 2) / ((np.pi * f4) - (np.pi * f3))) *
                     np.sinc(np.pi * f3 * t) ** 2)
            term3 = ((((np.pi * f2) ** 2) / ((np.pi * f2) - (np.pi * f1))) *
                     np.sinc(np.pi * f2 * t) ** 2)
            term4 = ((((np.pi * f1) ** 2) / ((np.pi * f2) - (np.pi * f1))) *
                     np.sinc(np.pi * f1 * t) ** 2)
