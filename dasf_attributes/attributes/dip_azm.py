...
class DipAzm(BaseAttributes):
    ...
    def gradient_dips(self, darray, dip_factor=10, kernel=(3, 3, 3),
                      preview=None):
        """
        Description
        -----------
        Compute Inline and Crossline Dip from the Inline, Crossline, &
            Z Gradients
        ...
        if kernel is not None:
            ...
            if util.is_cupy_enabled(self._use_cuda):
                ...
                xl_dip = xl_dip.map_overlap(cundi.median_filter, depth=hw,
                                            boundary='reflect',
                                            dtype=darray.dtype, size=kernel)
            else:
                il_dip = il_dip.map_overlap(ndi.median_filter, depth=hw,
                                            boundary='reflect',
                                            dtype=darray.dtype, size=kernel)
                xl_dip = xl_dip.map_overlap(ndi.median_filter, depth=hw,
                                            boundary='reflect',
                                            dtype=darray.dtype, size=kernel)
    ...
    def gradient_structure_tensor(self, darray, kernel, preview=None):
        ...
        if util.is_cupy_enabled(self._use_cuda):
            gi2 = (gi * gi).map_overlap(cundi.uniform_filter, depth=hw,
                                        boundary='reflect',
                                        dtype=darray.dtype, size=kernel)
            gj2 = (gj * gj).map_overlap(cundi.uniform_filter, depth=hw,
                                        boundary='reflect',
                                        dtype=darray.dtype, size=kernel)
            gk2 = (gk * gk).map_overlap(cundi.uniform_filter, depth=hw,
                                        boundary='reflect',
                                        dtype=darray.dtype, size=kernel)
            gigj = (gi * gj).map_overlap(cundi.uniform_filter, depth=hw,
                                         boundary='reflect',
                                         dtype=darray.dtype, size=kernel)
            gigk = (gi * gk).map_overlap(cundi.uniform_filter, depth=hw,
                                         boundary='reflect',
                                         dtype=darray.dtype, size=kernel)
            gjgk = (gj * gk).map_overlap(cundi.uniform_filter, depth=hw,
                                         boundary='reflect',
                                         dtype=darray.dtype, size=kernel)
        else:
            gi2 = (gi * gi).map_overlap(ndi.uniform_filter, depth=hw,
                                        boundary='reflect',
                                        dtype=darray.dtype, size=kernel)
            gj2 = (gj * gj).map_overlap(ndi.uniform_filter, depth=hw,
                                        boundary='reflect',
                                        dtype=darray.dtype, size=kernel)
            gk2 = (gk * gk).map_overlap(ndi.uniform_filter, depth=hw,
                                        boundary='reflect',
                                        dtype=darray.dtype, size=kernel)
            gigj = (gi * gj).map_overlap(ndi.uniform_filter, depth=hw,
                                         boundary='reflect',
                                         dtype=darray.dtype, size=kernel)
            gigk = (gi * gk).map_overlap(ndi.uniform_filter, depth=hw,
                                         boundary='reflect',
                                         dtype=darray.dtype, size=kernel)
            gjgk = (gj * gk).map_overlap(ndi.uniform_filter, depth=hw,
                                         boundary='reflect',
                                         dtype=darray.dtype, size=kernel)
    ...
    def gst_2D_dips(self, darray, dip_factor=10, kernel=(3, 3, 3),
        ...
        gi2, gj2, gk2, gigj, gigk, gjgk = \
            self.gradient_structure_tensor(darray, kernel, preview=preview)

        il_dip = da.map_blocks(operation, gi2, gj2, gk2, gigj, gigk, gjgk,
                               axis=0, dtype=darray.dtype)
        xl_dip = da.map_blocks(operation, gi2, gj2, gk2, gigj, gigk, gjgk,
                               axis=1, dtype=darray.dtype)
